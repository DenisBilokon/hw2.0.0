public class Employee
{
    ...
    public DateTime DateOfHire
    {
        ...
        set
        {
            if (dateOfHire.Date < DateTime.Parse("04.03.2022"))
            {
                dateOfHire = value;
            }
            else
...
class Program
{
    ...
    {
        Employee one = new Employee("Denis", "Bilokon", DateTime.Parse("04.03.2022"));
        Console.WriteLine("Name is {0}, surname is {1}. Date of hire: {2}", one.Name, one.Surname, one.DateOfHire);
        double salary = one.Calculation(Employee.Position.Warden);
        Console.WriteLine("Salary is {0}, tax is {1}, position is {2}.", salary, salary * 0.34, Employee.Position.Warden);
    }
}